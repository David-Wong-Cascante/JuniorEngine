...
bool Junior::Application::Load()
{
	...
	try{
		Parser parser("testParse.txt", std::ios::out);
		parser.WriteValue("ComponentName");
		parser.StarScope();
		parser.WriteVariable("TextureMapID", 1);
		parser.EndScope();
	}
	...
	try{
		int textureMapID = 0;
		std::string name;
		Parser parser("testParse.txt");
		parser.ReadValue(name);
		parser.Skip("{");
		parser.ReadVariable("TextureMapID", textureMapID);
		parser.Skip("}");
		debug.Print("[DEBUG] Component Name: ");
		debug.PrintLn(name);
		debug.Print("[DEBUG] Texture Atlas ID: ");
		debug.PrintLn(textureMapID);
